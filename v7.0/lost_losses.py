...
def stack_loss(tensor):
    ...
    for k in range(tensor.shape[1]):
        w = tf.tile(tf.expand_dims(complex_list[:, k], axis=1), [
                    1, tensor.shape[1]])
        r = tf.abs(w - complex_list)
        rless = tf.cast(tf.less(r, stack_limit), tf.float32) * \
            tf.cast(tf.greater(r, precise_limit), tf.float32)
...
def polygon_loss(tensor):
    ...
    sa = (tensor_this[:, :, 0] + tensor_next[:, :, 0]) * \
        (tensor_next[:, :, 1] - tensor_this[:, :, 0])
