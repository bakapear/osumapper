...
def step3_read_maps_flow(params):
    ...
    # np.concatenate([, range(11, 11 + divisor + 1)])
    used_indices = [3, 4, 7, 8, 9, 10]
...
def step3_read_maps_pattern(params):
    ...
    for file in os.listdir(root):
        if file.endswith(".npz"):
            ...
            try:
                ...
                pattern_note_end = pattern_data[:,
                                                :, 1+key_count:1+key_count*2]
    ...
    for key_index in range(18):
        if len(result_avail_note_begin[key_index]) == 0:
            outdata.append([np.array([]), np.array([]), np.array([]), np.zeros(
                (0, pattern_length, 1 + key_index)), np.zeros((0, pattern_length, 1 + key_index))])
            continue
        ...
        outdata.append([anbf[begin_flag != 0], anef[end_flag != 0],
                        ahf[begin_flag != 0], pnb[begin_flag != 0], pne[end_flag != 0]])
...
def step3_save_pattern_dataset(data):
    ...
    for key_index in range(18):
        ...
        try:
            ...
            avail_note_begin, avail_note_end, avail_hold, pattern_note_begin, pattern_note_end = [
                []] * 5
        ...
        save_dict["{}k_pattern_note_begin".format(
            key_count)] = pattern_note_begin
