...
def read_npz(fn):
    with np.load(fn) as data:
        ...
        div_data = np.concatenate(
            [divisor_array(div_source), div_source2], axis=1)
...
def prefilter_data(train_data_unfiltered, div_data_unfiltered, train_labels_unfiltered):
    ...
    non_object_end_indices = [i for i, k in enumerate(
        train_labels_unfiltered) if True or k[4] == -1 and k[5] == -1]
    train_data = train_data_unfiltered[non_object_end_indices]
    div_data = div_data_unfiltered[non_object_end_indices]
    train_labels = train_labels_unfiltered[non_object_end_indices][:, [
        0, 1, 2, 3, 4]]
...
def preprocess_npzs(train_data_unfiltered, div_data_unfiltered, train_labels_unfiltered):
    train_data, div_data, train_labels = prefilter_data(
        train_data_unfiltered, div_data_unfiltered, train_labels_unfiltered)
    ...
    train_data2 = np.reshape(train_data, (-1, time_interval,
                                          train_data.shape[1], train_data.shape[2], train_data.shape[3]))
    div_data2 = np.reshape(div_data, (-1, time_interval, div_data.shape[1]))
    train_labels2 = np.reshape(
        train_labels, (-1, time_interval, train_labels.shape[1]))
...
def get_data_shape():
    for file in os.listdir(root):
        if file.endswith(".npz"):
            train_data_unfiltered, div_data_unfiltered, train_labels_unfiltered = read_npz(
...
def read_some_npzs_and_preprocess(npz_list):
    ...
    for fp in npz_list:
        if fp.endswith(".npz"):
            ...
            if _td.shape[1:] != train_shape[1:]:
                print("Warning: something wrong found in {}! shape = {}".format(
                    fp, _td.shape))
                continue
            td_list.append(_td)
            dd_list.append(_dd)
    ...
    train_data2, div_data2, train_labels2 = preprocess_npzs(
        train_data_unfiltered, div_data_unfiltered, train_labels_unfiltered)
    return train_data2, div_data2, train_labels2
...
def step2_train_model(model, PARAMS):
    ...
    early_stop = keras.callbacks.EarlyStopping(
        monitor='mean_absolute_error', patience=20)
    ...
    if len(train_file_list) < too_many_maps_threshold:
        ...
        (new_train_data, new_div_data, new_train_labels), (test_data, test_div_data,
                                                           test_labels) = train_test_split(train_data2, div_data2, train_labels2)
        ...
        for epoch in range(EPOCHS):
            ...
            for map_batch in range(np.ceil(len(train_file_list) / data_split_count).astype(int)):
                if map_batch == 0:
                    train_data2, div_data2, train_labels2 = read_some_npzs_and_preprocess(
                        train_file_list[map_batch * data_split_count: (map_batch+1) * data_split_count])
                    (new_train_data, new_div_data, new_train_labels), (test_data, test_div_data,
                                                                       test_labels) = train_test_split(train_data2, div_data2, train_labels2)
                else:
                    new_train_data, new_div_data, new_train_labels = read_some_npzs_and_preprocess(
                        train_file_list[map_batch * data_split_count: (map_batch+1) * data_split_count])
...
def step2_evaluate(model):
    ...
    test_predictions = model.predict([test_data, test_div_data]).reshape(
        (-1, time_interval, label_shape[1]))
