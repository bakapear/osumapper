...
def get_circle_hitsounds(map_json, **kwargs):
    ...
    hs_avail_flags = [sum([k*2**i for i, k in enumerate(hs_avail)])
                      for hs_avail in hs_avails]

    return_data = [np.array([hs_avail_flags[i]] + hs_groups[i])
                   for i in range(len(hs_groups))]
...
def get_hitsound_groups(hs_avail_flags, hs_data, note_metronome_group):
    ...
    note_avail = [(1 if i in note_metronome_group else 0)
                  for i in range(metronome_length)]
    note_avail_flags = sum([k*2**i for i, k in enumerate(note_avail)])

    possible_hs_groups = hs_data[bitwise_contains(
        hs_avail_flags, note_avail_flags)]
...
def apply_hitsounds(hs_avail_flags, hs_data, ticks, divisor=4, metronome_count=4):
    ...
    for tick in range(max_tick+1):
        ...
        if metronome_offset >= divisor * metronome_count or tick == max_tick:
            hs_group = get_random_hitsound_group(
                hs_avail_flags, hs_data, hs_current)
