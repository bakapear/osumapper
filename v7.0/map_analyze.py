...
def get_map_timing_array(map_json, length=-1, divisor=4):
    if length == -1:
        # it has an extra time interval after the last note
...
def get_all_ticks_and_lengths_from_ts(uts_array, ts_array, end_time, divisor=4):
    ...
    timestamps = [np.arange(uts["beginTime"], endtimes[i], uts["tickLength"] / divisor)
                  for i, uts in enumerate(uts_array)]
    ticks_from_uts = [list(range(len(timestamp_group)))
                      for timestamp_group in timestamps]
    tick_len = [[uts["tickLength"]] * len(np.arange(uts["beginTime"], endtimes[i],
                                                    uts["tickLength"] / divisor)) for i, uts in enumerate(uts_array)]
    # slider_len = [[ts["sliderLength"]] * len(np.arange(ts["beginTime"], endtimes[i], ts["tickLength"] / divisor)) for i, ts in enumerate(ts_array)];
    slider_len = [get_slider_len_ts(ts_array, timestamp)
                  for timestamp in np.concatenate(timestamps)]
