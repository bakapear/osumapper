    ...
    with open(json_name, encoding="utf-8") as map_json:
        ...
        if convert:
            mp3_file = os.path.join(
                file_dir, map_dict["general"]["AudioFilename"])
    ...
    for d in data:
        if d[3] == 1:
            transformed_data.append(
                [d[0], d[1], d[2], 1, 0, 0, 1, 0, d[4], d[5], d[6], d[7], d[8], d[9]])
        elif d[3] == 2:
            transformed_data.append(
                [d[0], d[1], d[2], 0, 1, 0, 0, 0, d[4], d[5], d[6], d[7], d[8], d[9]])
        elif d[3] == 3:
            transformed_data.append(
                [d[0], d[1], d[2], 0, 0, 1, 0, 0, d[4], d[5], d[6], d[7], d[8], d[9]])
        elif d[3] == 4:
            transformed_data.append(
                [d[0], d[1], d[2], 0, 0, 0, 1, 0, d[4], d[5], d[6], d[7], d[8], d[9]])
        elif d[3] == 5:
            transformed_data.append(
                [d[0], d[1], d[2], 0, 0, 0, 1, 0, d[4], d[5], d[6], d[7], d[8], d[9]])
        else:
            transformed_data.append(
                [d[0], d[1], d[2], 0, 0, 0, 0, 0, d[4], d[5], d[6], d[7], d[8], d[9]])
...
def read_and_save_osu_file(path, filename="saved", divisor=4):
    ...
    wav_data = read_wav_data(
        timestamps, wav_file, snapint=[-0.3, -0.2, -0.1, 0, 0.1, 0.2, 0.3], fft_size=128)
...
def read_and_save_osu_tester_file(path, filename="saved", json_name="mapthis.json", divisor=4):
    ...
    ticks, timestamps, tick_lengths, slider_lengths = get_all_ticks_and_lengths_from_ts(
        osu_dict["timing"]["uts"], osu_dict["timing"]["ts"], file_len, divisor=divisor)
    ...
    np.savez_compressed(filename, ticks=ticks,
                        timestamps=timestamps, wav=wav_data, extra=extra)
