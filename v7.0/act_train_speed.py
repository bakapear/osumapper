...
def read_npz_mm(fn):
    with np.load(fn) as data:
        ...
        train_labels = np.swapaxes(np.where([lsta < 0.1, (lsta >= 0.1) & (
            lsta < 0.5), (lsta >= 0.5) & (lsta < 1.5), lsta >= 1.5], 1, -1), 0, 1)
...
def prefilter_data_mm(train_data_unfiltered, div_data_unfiltered, train_labels_unfiltered):
    ...
    nonempty_labels = np.array(
        [i for i, label in enumerate(train_labels_unfiltered) if True])
    ...
    train_labels_normalized = (
        train_labels - train_glob_min)/(train_glob_max - train_glob_min) * 2 - 1
...
def preprocess_npzs(train_data_unfiltered, div_data_unfiltered, train_labels_unfiltered):
    train_data, div_data, train_labels = prefilter_data_mm(
...
def get_data_shape_mm():
    for file in os.listdir(root):
        if file.endswith(".npz"):
            train_data_unfiltered, div_data_unfiltered, train_labels_unfiltered = read_npz_mm(
...
def step3_train_model(PARAMS):
    ...
    if len(train_file_list) < too_many_maps_threshold:
        train_data2, div_data2, train_labels2 = read_some_npzs_and_preprocess_mm(
            train_file_list)
        ...
        for epoch in range(EPOCHS):
            for map_batch in range(np.ceil(len(train_file_list) / data_split_count).astype(int)):
                if map_batch == 0:
                    ...
                    new_train_data, new_div_data, new_train_labels = read_some_npzs_and_preprocess_mm(
                        train_file_list[map_batch * data_split_count: (map_batch+1) * data_split_count])
