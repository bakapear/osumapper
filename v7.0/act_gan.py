...
class KerasCustomMappingLayer(keras.layers.Layer):
    ...
    def __init__(self, extvar, output_shape=(None, None), *args, **kwargs):
        ...
        self.extvar_begin = tf.Variable(tf.convert_to_tensor(
            extvar["begin"], dtype=tf.int32), trainable=False)
        self.extvar_lmul = tf.Variable(tf.convert_to_tensor(
            [extvar["length_multiplier"]], dtype=tf.float32), trainable=False)
        self.extvar_nfse = tf.Variable(tf.convert_to_tensor(
            extvar["next_from_slider_end"], dtype=tf.bool), trainable=False)
        self.extvar_mtfd = tf.Variable(tf.convert_to_tensor(
            GAN_PARAMS["max_ticks_for_ds"], dtype=tf.float32), trainable=False)
        self.note_group_size = GAN_PARAMS["note_group_size"]

        self.extvar_spos = tf.Variable(
            tf.cast(tf.zeros((2, )), tf.float32), trainable=False)
        self.extvar_rel = tf.Variable(
            tf.cast(tf.zeros((6, self.note_group_size)), tf.float32), trainable=False)
        self.extvar_tickdiff = tf.Variable(
            tf.cast(tf.zeros((self.note_group_size,)), tf.float32), trainable=False)
    ...
    def set_extvar(self, extvar):
        ...
        self.extvar_tickdiff.assign(tf.convert_to_tensor(
            tick_diff[begin_offset: begin_offset + self.note_group_size], dtype=tf.float32))
...
def generative_model(in_params, out_params, loss_func='mse'):
    ...
    """
    model = keras.Sequential([
        # activation=tf.nn.elu, input_shape=(train_data.shape[1],)),
...
def mixed_model(generator, mapping_layer, discriminator, in_params):
    ...
    losses = [AlwaysZeroCustomLoss(), BoxCustomLoss(
        GAN_PARAMS["box_loss_border"], GAN_PARAMS["box_loss_value"]), GenerativeCustomLoss()]
