...
def read_npz(fn):
    with np.load(fn) as data:
        ...
        div_data = np.array(
            [divisor_array(k) + [ex1[i], ex2[i], ex3[i]] for i, k in enumerate(ticks)])
...
def step5_predict_notes(model, npz, params):
    ...
    test_data2 = np.reshape(test_data, (-1, time_interval,
    ...
    is_obj_pred = np.expand_dims(
        np.where(preds[:, 0] > borderline, 1, 0), axis=1)

    obj_type_pred = np.sign(
        preds[:, 1:4] - np.tile(np.expand_dims(np.max(preds[:, 1:4], axis=1), 1), (1, 3))) + 1
    others_pred = (
        1 + np.sign(preds[:, 4:test_predictions.shape[1]] + 0.5)) / 2
    another_pred_result = np.concatenate(
        [is_obj_pred, is_obj_pred * obj_type_pred, others_pred], axis=1)
...
def step5_convert_sliders(data, params):
    ...
    obj_indices = [i for i, k in enumerate(
        unfiltered_objs) if k == 1 or unfiltered_predictions[i, 4] == 1]
    ...
    for i in range(len(first_step_objs)):
        # not first_step_objs = slider end
        if skip_this or not first_step_objs[i]:
        ...
        if first_step_is_slider[i]:  # this one is a slider!!
            if i == first_step_objs.shape[0]-1:  # Last Note.
                ...
                continue
            # too long! end here
            if first_step_ticks[i+1] >= first_step_ticks[i] + slider_max_ticks + 1:
                new_obj_indices.append(i)
                slider_ticks.append(slider_max_ticks)
            else:
                ...
                new_obj_indices.append(i)
                slider_ticks.append(
                    max(1, first_step_ticks[i+1] - first_step_ticks[i]))
        else:  # not a slider!
            new_obj_indices.append(i)
