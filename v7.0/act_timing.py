...
def get_timed_osu_file(music_path, input_filename="assets/template.osu", output_filename="temp/timing.osu", game_mode=0, mania_key_count=None):
    ...
    osu_text = re.sub("{hp_drain}",           "{}".format(
        np.random.randint(0, 101) / 10), osu_text)
    if mania_key_count is None:
        osu_text = re.sub("{circle_size}",    "{}".format(
            np.random.randint(30, 51) / 10), osu_text)
    else:
        osu_text = re.sub("{circle_size}",    "{}".format(
            mania_key_count), osu_text)
    osu_text = re.sub("{overall_difficulty}", "{}".format(
        np.random.randint(50, 91) / 10), osu_text)
    osu_text = re.sub("{approach_rate}",      "{}".format(
        np.random.randint(70, 96) / 10), osu_text)
    osu_text = re.sub("{slider_velocity}",    "{}".format(
        np.random.randint(12, 26) / 10), osu_text)
    osu_text = re.sub("{tickLength}",
                      "{}".format(60000 / bpm), osu_text)
    osu_text = re.sub("{offset}",             "{}".format(
        int(offset)),     osu_text)
